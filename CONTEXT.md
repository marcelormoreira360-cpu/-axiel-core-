...
**Escopo atual:** patient-facing = tela do paciente + PDF do paciente + **Doc 1**. **Global = TUDO (patient-facing + interno/profissional/Laudo/agentes) — AUTORIZADO por Marcelo (02/09) como a frente 2, ainda NÃO iniciada.** TRAVA a lembrar na frente 2: os prompts do Laudo/Salvo/Aval devem continuar RACIOCINANDO em disfunção (a apresentação/copy profissional é que vira equilíbrio) — senão reintroduz o risco de agente ler "72 = bom". `Pontos de atenção` seguem em disfunção (decisão de Marcelo). i18n `neuroId.json` nos 3 locales (novas: ringCaption/indexBalanceSub/dysfunctionTooltip; reescritas: indexCaption/cardSubtitle/legendRanges; pyramidShareCaption mantida p/ o card interno). Rótulos do PDF do Doc 1 seguem hardcoded PT (i18n do PDF é follow-up antigo).
