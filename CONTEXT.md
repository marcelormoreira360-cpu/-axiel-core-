...
3. **PDF do paciente** (`buildNeuroIdPatientReportPdf` em `services/neuro-id-pdf-service.ts`): número herói em equilíbrio + anel Bio³ desenhado no pdfkit (`drawBio3Ring`, via `doc.path()` com arcos SVG — pdfkit suporta 'A'). Copy (`report-copy.ts` Beat 2/5 + legenda) reescrita para narrativa de equilíbrio, REVISADA por Aval + Termo (aprovado com ajustes, aplicados): direção como META, não fato futuro. Placeholder novo `{equilibrio}`. Smoke test `services/__tests__/neuro-id-patient-report-pdf.test.ts`. PDF **clínico** (`view=clinical`, `buildNeuroIdMapPdf`) permanece em disfunção/inalterado.
4. **Doc 1 persuasivo em EQUILÍBRIO (02/09/2026, mesma branch)** — `buildNeuroIdDoc1Pdf`: seção 2 troca a pirâmide (painel) pelo **Anel Bio³** em equilíbrio (nova `drawBio3RingPanel`, que reusa `drawBio3Ring`; índice geral e %s via `dysfunctionToBalance`; rodapé "Índice de equilíbrio X% · maior = melhor"). Cor/estado de cada fatia continua saindo da DISFUNÇÃO crua (dentro do `drawBio3Ring`). `drawPyramidPanel` REMOVIDA (código morto: o map clínico usa `drawPyramid`). **Camada de IA** (`modules/ai-insights/guardrails.ts`): a instrução `leitura_bio3` + a regra "MAPA BIO³" agora mandam a prosa do Doc 1 ser **qualitativa e SEM percentuais** (o anel carrega os números em equilíbrio; o modelo usa `priority_pillar` p/ dizer qual eixo pede cuidado e quais estão preservados) — DECISÃO Marcelo "prosa sem número" (evita contradição com relatórios já gerados). Interno intacto: `ai-insight-review-card.tsx` desenha só a `NeuroPyramid` (disfunção) e NÃO renderiza `leitura_bio3.descricao`. Smoke test `neuro-id-doc1-pdf.test.ts` mantido. FALTA (follow-up): anel de equilíbrio no PREVIEW on-screen `neuro-id-360-documents.tsx` (hoje seção 2 é só texto; exige passar o mapa numérico ao componente).

**Gates Salvo/Aval/Termo rodaram (02/09):** todos LIBERAM COM AJUSTE. Aplicado: (a) `guardrails.ts` endurecido (progresso=meta não fato; verbos-verde x verbos-vermelho banidos tratar/curar/restaurar/"responde melhor"; índice=número do modelo, nunca "% saudável"; âncora sempre dos eixos preservados, nunca do prioritário, sem afirmar "centros" cerebrais; segurança emocional não pode enfraquecer encaminhamento); (b) `report-copy.ts` — linha nova no disclaimer (Índice Bio³ = número do método, não medida de saúde) + Beat 6 (tenso/bloqueado) sem prognóstico de sintoma/medo. Rótulo mantido "34%" (decisão Marcelo; disclaimer cobre). **PENDENTE (item de segurança, virou task):** red flag emocional (ideação/PHQ-9 item 9) precisa injetar encaminhamento/linha de crise DETERMINÍSTICO no caminho do Doc 1 (hoje depende do modelo; `showSafeguard`/CRISIS_HOTLINE existem só no PDF por-scores fallback) — fazer com o Laudo antes do publish.

**Frente 2 (global) — camada PROFISSIONAL mostra os DOIS (02/09, EM ANDAMENTO na mesma branch):** feito nas superfícies de DISPLAY NUMÉRICO do app: (1) PDF clínico `buildNeuroIdMapPdf` (herói "Disfunção X% · Equilíbrio Y%" + "eq Y%" por eixo); (2) `patient-direction-panel` (prontuário+sessão, "eq Y%" por eixo e no índice). Motor/scoring/cor SEMPRE em disfunção crua. NÃO-ALVOS com razão: `neuro-pyramid` (mostra share, não índice), `patient-assessment-progress-panel` (pontos de questionário), `case-summary.ts` (input de prompt da IA + texto interno → fica em disfunção pela trava). FALTA decidir: parte "agentes/equipe" do TUDO = prompts do Laudo/Salvo/Aval **continuam raciocinando em disfunção** (a trava; recomendo NÃO mexer no raciocínio) + retreino "número alto=bom" é ops, não código.

**Escopo:** patient-facing = tela + PDF + Doc 1 (equilíbrio). Profissional = mostra os dois. TRAVA: prompts do Laudo/Salvo/Aval RACIOCINAM em disfunção; só a apresentação vira equilíbrio — senão reintroduz o risco de agente ler "72 = bom". `Pontos de atenção` seguem em disfunção (decisão de Marcelo). i18n `neuroId.json` nos 3 locales (novas: ringCaption/indexBalanceSub/dysfunctionTooltip; reescritas: indexCaption/cardSubtitle/legendRanges; pyramidShareCaption mantida p/ o card interno). Rótulos do PDF do Doc 1 seguem hardcoded PT (i18n do PDF é follow-up antigo).
