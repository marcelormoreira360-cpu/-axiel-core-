...
const SEGMENTS: { value: Segment; label: string; description: string }[] = [
  {
    ...
    description: "Todos os pacientes com status ativo e telefone cadastrado.",
  },
  {
    value: "inactive_30",
    label: "Inativos +30 dias",
    description: "Pacientes sem sessão há mais de 30 dias — ótimos para reativação.",
  },
  {
    value: "inactive_60",
    label: "Inativos +60 dias",
    description: "Pacientes sem sessão há mais de 60 dias — risco de abandono.",
  },
];
          ...
          <div className="bg-white dark:bg-[#161B26] rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-black/[.06] dark:border-white/[.08]">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-[15px] font-semibold text-[#0F1A2E] dark:text-[#E8E6E2]">
                  {step === "done" ? "Envio concluído" : "Envio em massa · WhatsApp"}
                </h2>
              </div>
            ...
            {step === "sending" && (
              <div className="flex flex-col items-center justify-center py-16 px-6 gap-4">
                ...
                <p className="text-[14px] font-medium text-[#0F1A2E] dark:text-[#E8E6E2]">Enviando mensagens…</p>
                <p className="text-[12px] text-[#A09E98] text-center">
                  Isso pode levar até alguns segundos por paciente. Não feche esta janela.
                </p>
              </div>
            )}
            ...
            {step === "done" && result && (
              <div className="p-5 space-y-4">
                  ...
                  <div>
                    ...
                    {result.failed > 0 && (
                      <p className="text-[12px] text-[#6B6A66] mt-[2px]">
                        {result.failed} falharam — verifique os números desses pacientes.
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { label: "Total", value: result.total, color: "#0F1A2E" },
                    { label: "Enviado", value: result.sent, color: "#0F6E56" },
                    { label: "Falhou", value: result.failed, color: result.failed > 0 ? "#EB5757" : "#A09E98" },
                  ].map((s) => (
                ...
                {result.failedNames.length > 0 && (
                  <div className="bg-red-50 dark:bg-red-900/10 rounded-[10px] p-3">
                    <p className="text-[11px] font-semibold text-red-600 mb-1">Falhas:</p>
              ...
              <div className="p-5 space-y-4">
                    ...
                    <div className="absolute z-50 top-full mt-1 left-0 right-0 bg-white dark:bg-[#1C2333] border border-black/[.08] dark:border-white/[.1] rounded-[10px] shadow-lg overflow-hidden">
                      {SEGMENTS.map((s) => (
                        ...
                        >
                          ...
                          <p className="text-[11px] text-[#A09E98] mt-[1px]">{s.description}</p>
                        </button>
                      ))}
                    </div>
                  )}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    maxLength={1000}
                    rows={5}
                    placeholder="Olá, {{nome}}! Sentimos sua falta. Que tal agendar uma nova sessão? 🌿"
                  ...
                  <div className="flex gap-2 mt-2 flex-wrap">
                    <span className="text-[10px] text-[#A09E98]">Inserir:</span>
                  ...
                  <div>
                    ...
                    {showPreview && (
                      <div className="mt-2 bg-[#E1F5EE] rounded-[10px] px-4 py-3">
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-[#0F6E56] mb-2">Como o paciente vai receber</p>
                ...
                {count === 0 && !loadingCount && (
                  <div className="flex items-center gap-2 px-3 py-2 bg-amber-50 dark:bg-amber-900/10 rounded-[8px] text-[12px] text-amber-700">
                    <AlertCircle className="w-4 h-4 shrink-0" />
                    Nenhum paciente encontrado neste segmento.
                  </div>
                )}
