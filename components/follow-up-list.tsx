...
export function FollowUpList({
  ...
  return (
    <div className="space-y-3">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1 rounded-full bg-white px-3 py-1 text-xs font-semibold text-black/45 ring-1 ring-axiel-line">
                  <ChannelIcon channel={followUp.channel} /> {followUp.channel === "none" ? "Reminder only" : followUp.channel.toUpperCase()}
                </span>
              </div>
              <h3 className="mt-3 text-xl font-semibold tracking-tight">{followUp.title}</h3>
              <p className="mt-1 text-sm text-black/55">{followUp.patients?.full_name ?? "Patient"} · {new Date(followUp.due_at).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })}</p>
                ...
                {sendAction && followUp.channel !== "none" && (
                  <form action={sendAction}>
                    ...
                    <Button variant="secondary" className="min-h-12 gap-2">
                      {followUp.channel === "email" ? <Mail className="h-4 w-4" /> : <MessageSquare className="h-4 w-4" />} Send
                    </Button>
                  </form>
                )}
      ...
      {followUps.length > 5 ? (
        <ViewDetails label={`View ${followUps.length - 5} more follow-ups`}>
          <div className="space-y-3">
      ...
      {followUps.length === 0 && (
        <EmptyState
          ...
          text="No follow-up reminders yet. Create one so the patient always knows the next step."
          href="/follow-ups"
