    ...
    <Card className={compact ? "p-5" : "p-6"}>
      <div className="flex items-start gap-3">
        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-axiel-soft dark:bg-white/[.04]">
          <Sparkles className="h-5 w-5 text-axiel-gold dark:text-[#9FE1CB]" />
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <span className="inline-flex w-fit items-center gap-2 rounded-full bg-axiel-soft dark:bg-white/[.04] px-3 py-1.5 text-xs font-semibold text-black/55">
              <ShieldCheck className="h-3.5 w-3.5" /> {t("placeholder")}
            </span>
          </div>
          <p className="mt-2 rounded-2xl bg-amber-50 dark:bg-amber-500/10 px-4 py-3 text-xs font-semibold text-amber-900/80 dark:text-amber-200/80">{AI_INSIGHT_LABEL}</p>
      ...
      <div className={compact ? "mt-4 grid gap-3" : "mt-5 grid gap-4 md:grid-cols-2"}>
        <div className="rounded-xl border border-axiel-line bg-white p-6 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md">
          <p className="text-sm font-semibold text-black/80 dark:text-white/80">{t("patterns")}</p>
          <div className="mt-3 grid gap-3">
            {(patterns.length ? patterns : [{ title: t("noPatternTitle"), text: t("noPatternText") }]).map((item) => (
              <div key={`${item.title}-${item.text}`}>
                <p className="text-sm font-semibold text-black/75 dark:text-white/75">{item.title}</p>
        ...
        <div className="rounded-xl border border-axiel-line bg-white p-6 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md">
          <p className="text-sm font-semibold text-black/80 dark:text-white/80">{t("nextSteps")}</p>
          <div className="mt-3 grid gap-3">
            {(nextSteps.length ? nextSteps : [{ title: t("keepUpdatedTitle"), text: t("keepUpdatedText") }]).map((item) => (
              <div key={`${item.title}-${item.text}`} className="flex gap-2">
                <ArrowRight className="mt-0.5 h-4 w-4 shrink-0 text-axiel-gold dark:text-[#9FE1CB]" />
                <div>
                  <p className="text-sm font-semibold text-black/75 dark:text-white/75">{item.title}</p>
                  <p className="mt-1 text-sm leading-5 text-black/50">{item.text}</p>
                </div>
