...
export function PatientOfferForm({
  ...
  return (
    <form action={action} className="space-y-4 rounded-xl border border-axiel-line bg-white p-6 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md">
      <div>
        <p className="text-sm font-semibold text-black/75 dark:text-white/75">Assign to patient</p>
      ...
      <label className="block text-sm font-semibold text-black/65">
        Notes
        <textarea name="notes" rows={3} placeholder="Optional internal note" className="mt-2 w-full rounded-2xl border border-axiel-line bg-white px-4 py-3 text-sm outline-none focus:border-black/25 dark:focus:border-white/25" />
      </label>
