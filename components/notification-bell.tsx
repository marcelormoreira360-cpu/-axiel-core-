...
const ITEMS: Array<{
...
}> = [
  {
    key: "insights",
    href: "/ai-insights",
    label: (n) => `${n} insight${n > 1 ? "s" : ""} aguardando review`,
    dot: "bg-[#0F6E56]",
    Icon: BrainCircuit,
  },
  {
    key: "lgpd",
    href: "/settings/lgpd",
    label: (n) => `${n} solicitaç${n > 1 ? "ões" : "ão"} LGPD pendente${n > 1 ? "s" : ""}`,
    dot: "bg-red-500",
    Icon: Shield,
  },
  {
    key: "followups",
    href: "/patients",
    label: (n) => `${n} follow-up${n > 1 ? "s" : ""} vencido${n > 1 ? "s" : ""}`,
    dot: "bg-amber-400",
    Icon: CalendarClock,
  },
  {
    key: "leads",
    href: "/leads",
    label: (n) => `${n} novo${n > 1 ? "s" : ""} lead${n > 1 ? "s" : ""} sem revisão`,
    dot: "bg-indigo-400",
  ...
  },
  {
    key: "forms",
    href: "/forms",
    label: (n) => `${n} formulário${n > 1 ? "s" : ""} aguardando resposta`,
    dot: "bg-sky-400",
    Icon: ClipboardCheck,
  },
];
...
export function NotificationBell() {
  ...
  const total =
    counts.insights + counts.lgpd + counts.followups + counts.leads + counts.forms;
  ...
  useEffect(() => {
    ...
    // Fallback polling every 60s (in case realtime is throttled)
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 z-50 bg-white dark:bg-[#0B0F17] border border-black/[.07] dark:border-white/[.08] rounded-[12px] shadow-lg overflow-hidden min-w-[240px]">
          <div className="px-[14px] py-[10px] border-b border-black/[.05] dark:border-white/[.06]">
            <p className="text-[11px] font-semibold text-[#0F1A2E] dark:text-[#E8E6E2]">
              Notificações
            </p>
            {total > 0 && (
          ...
          <div className="py-[6px]">
            {total === 0 ? (
              <p className="px-[14px] py-[10px] text-[12px] text-[#A09E98] dark:text-[#6B6A66]">
