...
export default async function IntegrationsPage({
  ...
  const { data: clinicSettings } = await supabase
    ...
    .maybeSingle();
  const settingsJson = (clinicSettings?.settings as Record<string, unknown> | null) ?? {};
  ...
  const toast = sp.success
    ? { type: "success", message:
        sp.success === "google"              ? t("toastGoogleConnected") :
        sp.success === "google_disconnected" ? t("toastGoogleDisconnected") :
        sp.success === "zoom"                ? t("toastZoomConnected") :
        sp.success === "zoom_disconnected"   ? t("toastZoomDisconnected") : "" }
    : sp.error
    ? { type: "error", message:
        sp.error === "google_denied" ? t("toastGoogleDenied") :
        sp.error === "zoom_denied"   ? t("toastZoomDenied") :
        t("toastError") }
    : null;
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white dark:bg-[#161B26] border border-black/[.07] dark:border-white/[.08] rounded-[14px] p-[20px]">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[14px] font-medium text-[#0F1A2E] dark:text-[#E8E6E2]">{t("googleTitle")}</p>
                <p className="text-[12px] text-[#A09E98] mt-[1px]">{t("googleDesc")}</p>
              </div>
            </div>
            <StatusBadge connected={googleStatus.connected} label={googleStatus.connected ? t("connected") : t("notConnected")} />
          </div>
          ...
          {googleStatus.connected && (
            <div className="mt-3 flex items-center gap-2 bg-[#F4F3EF] dark:bg-white/[.04] rounded-[8px] px-[12px] py-[8px]">
              ...
              <span className="text-[12px] text-[#6B6A66] dark:text-[#9E9C97]">
                {t("calendar")} <strong className="text-[#0F1A2E] dark:text-[#E8E6E2]">{googleStatus.calendar_id ?? "primary"}</strong>
              </span>
            </div>
          )}
        ...
        <div className="bg-white dark:bg-[#161B26] border border-black/[.07] dark:border-white/[.08] rounded-[14px] p-[20px]">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[14px] font-medium text-[#0F1A2E] dark:text-[#E8E6E2]">{t("zoomTitle")}</p>
                <p className="text-[12px] text-[#A09E98] mt-[1px]">{t("zoomDesc")}</p>
              </div>
            </div>
            <StatusBadge connected={zoomStatus.connected} label={zoomStatus.connected ? t("connected") : t("notConnected")} />
          </div>
          ...
          {zoomStatus.connected ? (
            ...
            <div className="mt-3 flex items-center gap-2 bg-[#FFF8E1] dark:bg-yellow-500/[.08] rounded-[8px] px-[12px] py-[8px]">
              ...
              <span className="text-[12px] text-[#92400E] dark:text-yellow-400">
                {t.rich("zoomEnvHint", { code: (c) => <code className="bg-white dark:bg-[#1C2333] px-1 rounded text-[11px]">{c}</code> })}
              </span>
            </div>
          )}
        ...
        <div className="bg-white dark:bg-[#161B26] border border-black/[.07] dark:border-white/[.08] rounded-[14px] p-[20px]">
          <div className="flex items-start justify-between gap-4 mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[14px] font-medium text-[#0F1A2E] dark:text-[#E8E6E2]">{t("icalTitle")}</p>
                <p className="text-[12px] text-[#A09E98] mt-[1px]">{t("icalDesc")}</p>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-[#161B26] border border-black/[.07] dark:border-white/[.08] rounded-[14px] p-[20px] hover:border-black/15 transition cursor-pointer">
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[14px] font-medium text-[#0F1A2E] dark:text-[#E8E6E2]">{t("nfseTitle")}</p>
                  <p className="text-[12px] text-[#A09E98] mt-[1px]">{t("nfseDesc")}</p>
                </div>
              </div>
          ...
          <div className="bg-white dark:bg-[#161B26] border border-black/[.07] dark:border-white/[.08] rounded-[14px] p-[20px] hover:border-black/15 transition cursor-pointer">
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[14px] font-medium text-[#0F1A2E] dark:text-[#E8E6E2]">{t("hotmartTitle")}</p>
                  <p className="text-[12px] text-[#A09E98] mt-[1px]">{t("hotmartDesc")}</p>
                </div>
              </div>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-black/30 mt-1 shrink-0"><path d="m9 18 6-6-6-6"/></svg>
            </div>
        ...
        <div className="bg-white dark:bg-[#161B26] border border-black/[.07] dark:border-white/[.08] rounded-[14px] p-[20px]">
          <div className="flex items-center gap-3 mb-1">
            <div className="w-10 h-10 rounded-[10px] border border-black/[.07] flex items-center justify-center shrink-0 bg-white dark:bg-[#1C2333]">
              {/* Google "G" icon */}
            ...
            <div>
              <p className="text-[14px] font-medium text-[#0F1A2E] dark:text-[#E8E6E2]">{t("reviewsTitle")}</p>
              <p className="text-[12px] text-[#A09E98] mt-[1px]">{t("reviewsDesc")}</p>
            </div>
          </div>
