...
export function HotmartForm({ clinicId: _clinicId, webhookUrl, hasToken }: Props) {
  const t = useTranslations("settings.hotmart");
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Instruções */}
      <Card className="p-6">
        <h2 className="mb-1 text-lg font-semibold">{t("howToTitle")}</h2>
        <ol className="mt-3 space-y-3 text-sm text-black/65">
          <li className="flex gap-2">
            ...
            {t.rich("step1", { b: (c) => <strong>{c}</strong> })}
          </li>
          <li className="flex gap-2">
            ...
            {t.rich("step2", { b: (c) => <strong>{c}</strong> })}
          </li>
          <li className="flex gap-2">
            ...
            {t.rich("step3", { b: (c) => <strong>{c}</strong> })}
          </li>
          <li className="flex gap-2">
            ...
            {t.rich("step4", { b: (c) => <strong>{c}</strong> })}
          </li>
        </ol>
      </Card>
      ...
      {/* URL do webhook */}
      <Card className="p-6">
        <h2 className="mb-1 text-lg font-semibold">{t("webhookTitle")}</h2>
        <p className="mb-3 text-sm text-black/50">{t("webhookDesc")}</p>
      ...
      <form onSubmit={handleSubmit}>
        <Card className="p-6">
          <h2 className="mb-1 text-lg font-semibold">{t("tokenTitle")}</h2>
          ...
          <div className="mt-4 flex items-center gap-4">
            <Button type="submit" disabled={isPending}>
              {isPending ? t("saving") : t("saveToken")}
            </Button>
            {saved && <span className="text-sm text-green-600">{t("tokenSaved")}</span>}
