...
export function PractitionersList({ practitioners }: Props) {
  const t = useTranslations("settings.practitioners");
  ...
  if (practitioners.length === 0) {
    return (
      <div className="bg-white border border-black/[.07] rounded-[12px] p-8 text-center space-y-3">
        <p className="text-[14px] font-medium text-[#0F1A2E]">{t("emptyTitle")}</p>
  ...
  return (
    <div className="space-y-3">
      {practitioners.map((p) => {
        ...
        const displayLabel = p.display_name ?? p.full_name ?? t("defaultName");
        ...
        return (
          <div key={p.user_id} className="bg-white border border-black/[.07] rounded-[12px] px-5 py-4">
            {!isEditing ? (
              ...
              <div className="space-y-3">
                <p className="text-[11px] font-semibold text-[#A09E98] uppercase tracking-[.1em]">{t("editTitle")}</p>
                <div>
                  <label className="text-[11px] font-medium text-[#6B6A66] mb-1 block">{t("displayName")}</label>
                  <input
                    value={form?.display_name ?? ""}
                    onChange={(e) => setFormState((prev) => ({ ...prev, [p.user_id]: { ...prev[p.user_id], display_name: e.target.value } }))}
                    placeholder={p.full_name ?? t("displayNamePlaceholder")}
                ...
                <div>
                  <label className="text-[11px] font-medium text-[#6B6A66] mb-1 block">{t("specialty")}</label>
                  <input
                ...
                <div>
                  <label className="text-[11px] font-medium text-[#6B6A66] mb-1 block">{t("bio")}</label>
                  <textarea
                    value={form?.bio ?? ""}
                ...
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  ...
                  />
                  <span className="text-[12px] text-[#0F1A2E]">{t("bookableLabel")}</span>
                </label>
