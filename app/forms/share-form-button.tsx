    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-[400px] bg-white dark:bg-[#111827] rounded-[16px] border border-black/[.08] dark:border-white/[.08] shadow-xl p-5">
            {/* Header */}
            <div className="flex items-start justify-between mb-4">
              <div>
                <p className="text-[11px] font-medium tracking-[.08em] uppercase text-[#A09E98]">{t("generateLink")}</p>
                <h2 className="text-[15px] font-medium text-[#0F1A2E] dark:text-[#E8E6E2] mt-[2px] leading-tight">{templateName}</h2>
              </div>
              <button
                type="button"
                onClick={handleClose}
            ...
            {!generatedUrl ? (
              <>
                ...
                <div className="relative mb-2">
                  ...
                  <input
                    ...
                    autoComplete="off"
                    className="w-full pl-[30px] pr-3 py-[8px] rounded-[8px] border border-black/[.10] dark:border-white/[.10] text-[13px] text-[#0F1A2E] dark:text-[#E8E6E2] placeholder:text-[#D3D1C7] outline-none focus:border-[#0F6E56] transition"
                  />
                </div>

                {/* Patient list */}
                <div className="max-h-[220px] overflow-y-auto rounded-[8px] border border-black/[.07] dark:border-white/[.07] divide-y divide-black/[.04] dark:divide-white/[.04]">
                  {filtered.length === 0 ? (
                    ...
                    filtered.map((p) => (
                      <button
                        ...
                        className="w-full text-left px-[12px] py-[9px] hover:bg-[#F4F3EF] dark:hover:bg-white/[.06] transition flex items-center gap-[8px] disabled:opacity-50"
                      >
              ...
              <>
                ...
                <div className="flex items-center gap-[6px] bg-[#F4F3EF] dark:bg-white/[.06] rounded-[8px] px-[10px] py-[8px]">
                  <p className="text-[11px] text-[#0F1A2E] dark:text-[#E8E6E2] flex-1 truncate font-mono">{generatedUrl}</p>
                  <button
                    type="button"
                    onClick={handleCopy}
                ...
                <button
                  ...
                  className="mt-3 text-[11px] text-[#A09E98] hover:text-[#0F1A2E] dark:hover:text-[#E8E6E2] transition"
                >
                  {t("selectOther")}
                </button>
              </>
