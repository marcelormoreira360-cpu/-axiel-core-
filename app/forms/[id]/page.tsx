    ...
    <Shell>
      ...
      <div className="flex items-center justify-between mb-[24px]">
        <div className="flex items-center gap-[10px]">
          ...
          <div>
            <h1 className="text-[18px] font-medium tracking-[-0.025em] text-[#0F1A2E] dark:text-[#E8E6E2]">{template.name}</h1>
      ...
      <div className="grid gap-[18px] xl:grid-cols-[1fr_340px]">
        ...
        <div className="space-y-[10px]">
          {template.assessment_sections.map((section) => (
            <div key={section.id} className="bg-white dark:bg-[#111827] border border-black/[.07] dark:border-white/[.07] rounded-[12px] overflow-hidden">
              <div className="flex items-center justify-between px-[14px] py-[9px] bg-[#F4F3EF] dark:bg-white/[.06]">
                <p className="text-[11px] font-medium tracking-[.06em] uppercase text-[#6B6A66] dark:text-[#9E9C97]">
                  {section.title}
                </p>
              ...
              <div className="divide-y divide-black/[.04] dark:divide-white/[.04]">
                {section.assessment_questions.map((q) => (
                  <div key={q.id} className="flex items-center justify-between px-[14px] py-[9px] gap-[12px]">
                    <p className="text-[12px] text-[#0F1A2E] dark:text-[#E8E6E2] flex-1">{q.text}</p>
        ...
        <div className="space-y-[12px]">
          <div className="bg-white dark:bg-[#111827] border border-black/[.07] dark:border-white/[.07] rounded-[12px] px-[16px] py-[14px]">
            <p className="text-[11px] font-medium text-[#6B6A66] dark:text-[#9E9C97] mb-[10px]">{t("fillFor")}</p>
            <FormPatientPicker patients={patients} templateId={id} />
          </div>

          <div className="bg-white dark:bg-[#111827] border border-black/[.07] dark:border-white/[.07] rounded-[12px] px-[16px] py-[14px]">
            <p className="text-[11px] font-medium text-[#6B6A66] dark:text-[#9E9C97] mb-[10px]">{t("sendLink")}</p>
            <FormInvitationPanel patients={patients} templateId={id} />
          </div>

          <div className="bg-white dark:bg-[#111827] border border-[#0F6E56]/20 rounded-[12px] px-[16px] py-[14px]">
            <PublicCaptureLinkPanel templateId={id} />
          </div>

          {template.instructions && (
            <div className="bg-[#F4F3EF] dark:bg-white/[.06] rounded-[12px] px-[14px] py-[12px]">
              <p className="text-[10px] font-medium text-[#6B6A66] dark:text-[#9E9C97] mb-[6px]">{t("instructions")}</p>
              <p className="text-[12px] text-[#0F1A2E] dark:text-[#E8E6E2] leading-relaxed">{template.instructions}</p>
            </div>
          )}
        </div>
      </div>
