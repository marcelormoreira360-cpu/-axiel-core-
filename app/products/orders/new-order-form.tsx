...
export function NewOrderForm({ products, patients }: { products: Product[]; patients: Patient[] }) {
  ...
  function submit() {
    setError(null);
    if (cart.length === 0) { setError(t("errNoProducts")); return; }
  ...
  return (
    <div className="max-w-2xl space-y-5">
      {/* Paciente */}
      <div>
        <label className="text-[12px] font-medium text-[#6B6A66] dark:text-[#9E9C97] mb-1.5 block">{t("patientLabel")}</label>
        <select
          value={patientId}
      ...
      {/* Adicionar produto */}
      <div>
        <label className="text-[12px] font-medium text-[#6B6A66] dark:text-[#9E9C97] mb-1.5 block">{t("productsLabel")}</label>
          ...
          {cart.map((l) => (
            <div key={l.product_id} className="flex items-center gap-3 px-4 py-3 border-b border-axiel-line last:border-b-0">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-[#A09E98]">{t("each", { price: money(l.unit_price_cents) })}</p>
              </div>
              <input
                type="number"
                min={1}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-[12px] font-medium text-[#6B6A66] dark:text-[#9E9C97] mb-1.5 block">{t("taxLabel")}</label>
          <input
            value={taxStr}
        ...
        <div>
          <label className="text-[12px] font-medium text-[#6B6A66] dark:text-[#9E9C97] mb-1.5 block">{t("notesLabel")}</label>
          <input
            value={notes}
      ...
      <div className="border-t border-axiel-line pt-4 space-y-1">
        <div className="flex justify-between text-[13px] text-[#6B6A66] dark:text-[#9E9C97]"><span>{t("subtotal")}</span><span>{money(subtotal)}</span></div>
        {taxCents > 0 && <div className="flex justify-between text-[13px] text-[#6B6A66] dark:text-[#9E9C97]"><span>{t("taxLine")}</span><span>{money(taxCents)}</span></div>}
        <div className="flex justify-between text-[15px] font-semibold text-[#0F1A2E] dark:text-[#E8E6E2]"><span>{t("total")}</span><span>{money(total)}</span></div>
      </div>
