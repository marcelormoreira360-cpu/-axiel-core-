...
function ptDate(iso: string | null | undefined, locale: string) {
  if (!iso) return "—";
  return new Date(iso).toLocaleDateString(locale, { day: "2-digit", month: "2-digit", year: "numeric" });
}

function ptDateTime(iso: string | null | undefined, locale: string) {
  if (!iso) return "—";
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!patient) return new Response(t("record.notFound"), { status: 404 });
  ...
  const doc = new PDFDocument({
    ...
    size: "A4",
    info: {
      Title: t("record.title", { name: patient.full_name }),
      Author: clinic.name,
      Subject: t("record.subject"),
    },
  });
  ...
  doc.fillColor("rgba(255,255,255,0.6)").font("Helvetica").fontSize(10).text(t("record.headerSubtitle"), 56, 44);
  doc.fillColor("rgba(255,255,255,0.4)").fontSize(9).text(t("generatedAt", { date: new Date().toLocaleString(locale) }), 56, 58);
  doc.y = 100;
  ...
  if (patient.email)          metaLines.push(t("record.email", { value: patient.email }));
  if (patient.phone)          metaLines.push(t("record.phone", { value: patient.phone }));
  if (patient.date_of_birth)  metaLines.push(age !== null ? t("record.birthAge", { value: ptDate(patient.date_of_birth, locale), age }) : t("record.birth", { value: ptDate(patient.date_of_birth, locale) }));
  metaLines.push(t("record.statusLine", { value: patient.status === "active" ? t("record.statusActive") : patient.status === "inactive" ? t("record.statusInactive") : t("record.statusArchived") }));
  metaLines.push(t("record.registeredLine", { value: ptDate(patient.created_at, locale) }));
  ...
  const RISK_LABEL: Record<string, string> = {
    none:   t("record.riskNone"),
    low:    t("record.riskLow"),
    medium: t("record.riskMedium"),
    high:   t("record.riskHigh"),
  };
  ...
  // Score
  doc.fillColor("#6b7280").font("Helvetica").fontSize(8).text(t("record.engagement"), 64, stripY + 5);
  ...
  // Risk
  doc.fillColor("#6b7280").font("Helvetica").fontSize(8).text(t("record.churnRisk"), 180, stripY + 5);
  doc.fillColor(riskColor).font("Helvetica-Bold").fontSize(10).text(RISK_LABEL[engagement.churnRisk] ?? "—", 180, stripY + 15);

  // Last session
  doc.fillColor("#6b7280").font("Helvetica").fontSize(8).text(t("record.lastSession"), 300, stripY + 5);
  const lastSessionText = engagement.daysSinceLastSession === null
    ? "—"
    : engagement.daysSinceLastSession === 0 ? t("record.today")
    : engagement.daysSinceLastSession === 1 ? t("record.yesterday")
    : t("record.daysAgo", { days: engagement.daysSinceLastSession });
  doc.fillColor("#111827").font("Helvetica-Bold").fontSize(9).text(lastSessionText, 300, stripY + 15);

  // Sessions 90d
  doc.fillColor("#6b7280").font("Helvetica").fontSize(8).text(t("record.sessions90"), 420, stripY + 5);
  ...
  // ── General notes ─────────────────────────────────────────────────────────────
  if (patient.notes) {
    sectionHeading(doc, t("record.generalNotes"));
  ...
  // ── Session history ───────────────────────────────────────────────────────────
  sectionHeading(doc, t("record.sessionHistory", { count: appointments.length }));

  if (appointments.length === 0) {
    doc.fillColor("#9ca3af").font("Helvetica").fontSize(9).text(t("record.noSessions"), 56, doc.y);
    doc.moveDown(0.5);
  } else {
    const cols = [
      { x: 56,  w: 104, label: t("record.colDateTime") },
      { x: 164, w: 160, label: t("record.colSessionType") },
      { x: 328, w: 55,  label: t("record.colDuration") },
      { x: 387, w: 60,  label: t("record.colStatus") },
      { x: 451, w: 92,  label: t("record.colNotes") },
    ];
    const hY = doc.y;
    ...
    const STATUS_PT: Record<string, string> = {
      completed: t("record.apptCompleted"),
      scheduled: t("record.apptScheduled"),
      confirmed: t("record.apptConfirmed"),
      cancelled: t("record.apptCancelled"),
      no_show:   t("record.apptNoShow"),
    };
    ...
    sorted.slice(0, 60).forEach((appt, i) => {
      ...
      doc.text(ptDateTime(appt.starts_at, locale), 60, rowY, { width: 100 });
      doc.text(stName, 168, rowY, { width: 156 });
      doc.text(t("record.minutes", { n: appt.duration_minutes }), 332, rowY, { width: 51 });
    ...
    if (appointments.length > 60) {
      ...
      doc.fillColor("#9ca3af").font("Helvetica").fontSize(8)
        .text(t("record.moreSessions", { count: appointments.length - 60 }));
    }
  }
  ...
  if (recordsWithNotes.length > 0) {
    sectionHeading(doc, t("record.sessionNotes", { count: recordsWithNotes.length }));
    ...
    recordsWithNotes.slice(0, 20).forEach((rec) => {
      ...
      doc.fillColor("#065f46").font("Helvetica-Bold").fontSize(8)
        .text(t("record.sessionOn", { date: ptDate(apptDate, locale) }), 60, noteY + 3);
      doc.y = noteY + 16;

      if (rec.soap_mode) {
        const soapFields: Array<[string, string | null]> = [
          [t("record.soapSubjective"), rec.subjective],
          [t("record.soapObjective"), rec.objective],
          [t("record.soapAssessment"), rec.assessment_note],
          [t("record.soapPlan"), rec.plan],
        ];
    ...
    if (recordsWithNotes.length > 20) {
      doc.fillColor("#9ca3af").font("Helvetica").fontSize(8)
        .text(t("record.moreNotes", { count: recordsWithNotes.length - 20 }));
    }
  }

  // ── Assessment responses ──────────────────────────────────────────────────────
  if (assessmentResponses.length > 0) {
    sectionHeading(doc, t("record.assessments", { count: assessmentResponses.length }));
    ...
    sorted.forEach((resp, i) => {
      ...
      const name = resp.assessment_templates?.name ?? t("record.formFallback");
  ...
  if (activePrescriptions.length > 0) {
    sectionHeading(doc, t("record.prescriptions"));
    ...
    const renderRxGroup = (label: string, items: typeof activePrescriptions) => {
      ...
      items.forEach((rx, i) => {
        ...
        if (rx.start_date) details.push(t("record.rxStart", { date: ptDate(rx.start_date, locale) }));
        if (rx.end_date)   details.push(t("record.rxEnd", { date: ptDate(rx.end_date, locale) }));
    ...
    renderRxGroup(t("record.medications"), medications);
    renderRxGroup(t("record.supplements"),  supplements);
  }
  ...
  if (exams.length > 0) {
    sectionHeading(doc, t("record.exams", { count: exams.length }));
    ...
    sortedExams.forEach((exam) => {
      ...
      doc.fillColor("#374151").font("Helvetica-Bold").fontSize(9)
        .text(`${ptDate(exam.exam_date, locale)}${exam.lab_name ? `  ·  ${exam.lab_name}` : ""}`, 60, examY + 3);
      doc.y = examY + 16;

      if (exam.exam_results.length === 0) {
        doc.fillColor("#9ca3af").font("Helvetica").fontSize(8).text(t("record.noExamResults"), 60, doc.y);
        doc.moveDown(0.4);
        return;
      }
      ...
      exam.exam_results.forEach((r, i) => {
        ...
        const STATUS_LABEL: Record<string, string> = { low: t("record.biomarkerLow"), high: t("record.biomarkerHigh"), normal: t("record.biomarkerNormal"), unknown: "—" };
        ...
        const ref = r.ref_min != null && r.ref_max != null
          ? t("record.ref", { min: r.ref_min, max: r.ref_max, unit: r.unit ? ` ${r.unit}` : "" })
          : "";
    ...
    if (examsWithResults.length === 0) {
      doc.moveDown(-0.3);
      doc.fillColor("#9ca3af").font("Helvetica").fontSize(8).text(t("record.examsNoBiomarkers"));
      doc.moveDown(0.3);
    }
  }
  ...
  if (allInsights.length > 0) {
    sectionHeading(doc, finalInsights.length > 0 ? t("record.insightsApproved", { count: allInsights.length }) : t("record.insightsRecent", { count: allInsights.length }));
    ...
    allInsights.slice(0, 10).forEach((insight) => {
      ...
      const statusLabel: Record<string, string> = { final: t("record.insightFinal"), pending_review: t("record.insightPending"), needs_changes: t("record.insightNeedsChanges"), archived: t("record.insightArchived") };
      doc.rect(56, insightY, 487, 14).fillColor("#f0fdf4").fill();
      doc.fillColor("#065f46").font("Helvetica-Bold").fontSize(8)
        .text(ptDate(insight.created_at, locale), 60, insightY + 3, { continued: true, width: 200 });
      doc.fillColor("#9ca3af").font("Helvetica").fontSize(7.5)
      ...
      if (currentStatus) {
        doc.fillColor("#374151").font("Helvetica-Bold").fontSize(8).text(t("record.currentStatus"), 60, doc.y, { continued: true });
        doc.fillColor("#4b5563").font("Helvetica").fontSize(8)
      ...
      if (output.patterns_and_correlations?.length) {
        output.patterns_and_correlations.slice(0, 2).forEach((pat) => {
          ...
          doc.fillColor("#374151").font("Helvetica-Bold").fontSize(8)
            .text(t("record.pattern", { title: pat.title }), 60, doc.y);
          doc.fillColor("#6b7280").font("Helvetica").fontSize(8)
      ...
      if (insight.reviewer_notes) {
        doc.fillColor("#065f46").font("Helvetica-Bold").fontSize(8)
          .text(t("record.reviewerNote"), 60, doc.y, { continued: true });
        doc.fillColor("#374151").font("Helvetica").fontSize(8)
    ...
    if (allInsights.length > 10) {
      doc.fillColor("#9ca3af").font("Helvetica").fontSize(8)
        .text(t("record.moreInsights", { count: allInsights.length - 10 }));
    }
  }
  ...
  doc.fillColor("#d1d5db").font("Helvetica").fontSize(8)
    .text(
      t("record.footer", { clinic: clinic.name }),
      { align: "center" },
    );
