...
export default async function TrendsPage() {
  ...
  return (
    <Shell>
      ...
      <div className="bg-[#E1F5EE] border border-[#0F6E56]/20 rounded-[12px] px-[16px] py-[12px] mb-[20px]">
        <p className="text-[12px] text-[#0B5A45] leading-relaxed">{t("privacyBanner")}</p>
      </div>

      {rows.length === 0 ? (
        <div className="bg-white border border-black/[.07] rounded-[12px] px-[20px] py-[40px] text-center">
          <p className="text-[13px] text-[#6B6A66] mb-[4px]">{t("emptyTitle")}</p>
        ...
        <div className="space-y-[20px]">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-[10px]">
            <div className="bg-white border border-black/[.07] rounded-[12px] px-[16px] py-[14px]">
              ...
              <p className="text-[22px] font-medium text-[#0F1A2E] mt-[2px]">{totalConsented}</p>
            </div>
            <div className="bg-white border border-black/[.07] rounded-[12px] px-[16px] py-[14px]">
              <p className="text-[11px] text-[#A09E98]">{t("kpiCities")}</p>
              <p className="text-[22px] font-medium text-[#0F1A2E] mt-[2px]">{distinctCities}</p>
            </div>
            <div className="bg-white border border-black/[.07] rounded-[12px] px-[16px] py-[14px]">
              <p className="text-[11px] text-[#A09E98]">{t("kpiGroups")}</p>
              <p className="text-[22px] font-medium text-[#0F1A2E] mt-[2px]">{rows.length}</p>
            </div>
          </div>

          {/* Por faixa etária */}
          <section className="bg-white border border-black/[.07] rounded-[12px] px-[18px] py-[16px]">
              ...
              {byAgeBand.map((b) => {
                ...
                return (
                  <div key={b.age_band} className="flex items-center gap-[10px]">
                    <span className="text-[12px] text-[#6B6A66] w-[90px] shrink-0">{ageLabel(b.age_band)}</span>
                    <div className="flex-1 h-[8px] bg-[#F4F3EF] rounded-full overflow-hidden">
                      <div className="h-full bg-[#0F6E56] rounded-full" style={{ width: `${pct}%` }} />
                    </div>
                    <span className="text-[12px] text-[#0F1A2E] tabular-nums w-[58px] text-right shrink-0">
                      {b.patient_count} · {pct}%
                    </span>
                  </div>
                );
          ...
          {/* Tabela por região */}
          <section className="bg-white border border-black/[.07] rounded-[12px] overflow-hidden">
            <p className="text-[11px] font-medium tracking-[.07em] uppercase text-[#A09E98] px-[18px] pt-[16px] pb-[10px]">
              {t("byRegion")}
            </p>
            <table className="w-full text-[12px]">
              <thead>
                <tr className="text-[#A09E98] border-b border-black/[.06]">
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={i} className="border-b border-black/[.04] last:border-0">
                    <td className="px-[18px] py-[8px] text-[#0F1A2E]">{r.city ?? "—"}</td>
                    <td className="px-[12px] py-[8px] text-[#6B6A66]">{r.state ?? "—"}</td>
                    <td className="px-[12px] py-[8px] text-[#6B6A66]">{ageLabel(r.age_band)}</td>
                    <td className="px-[18px] py-[8px] text-right text-[#0F1A2E] tabular-nums">{r.patient_count}</td>
                  </tr>
                ))}
              </tbody>
            </table>
